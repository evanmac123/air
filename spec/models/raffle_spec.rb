...
describe Raffle do
  ...
  let(:raffle) { FactoryGirl.create(:raffle, demo: demo) }

  describe "#pick_winners" do
    it "should pick required number of winners" do
      users = FactoryGirl.create_list(:user, 3, :with_tickets, demo: demo)
    ...
    it "puts winners in blacklist" do
      users = FactoryGirl.create_list(:user, 2, :with_tickets, demo: demo)
    ...
    it "should not repeat winner" do
      users = FactoryGirl.create_list(:user, 6, :with_tickets, demo: demo)
  ...
  describe "#repick_winner" do
    ...
    it "does not add duplicate winners" do
      users = FactoryGirl.create_list(:user, 1, :with_tickets, demo: demo)
      winners = raffle.pick_winners(1)
      bad_winner = winners.first
  ...
  describe "#show_start?" do
    it "creates a user_in_raffle_info for user" do
      user = FactoryGirl.create(:user, :with_tickets, demo: demo)
