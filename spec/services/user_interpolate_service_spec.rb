...
describe UserInterpolateService do
  ...
  describe "#interpolate_airbo_access_link" do
    it "interpolates {{link_to_airbo}} for the user's airbo access link" do
      ...
      expected_result = "Visit <a href='http://www.example.com/invitations/#{user.invitation_code}?demo_id=1&email_type=&tile_token=#{EmailLink.generate_token(user)}&user_id=#{user.id}'>Airbo</a>"
