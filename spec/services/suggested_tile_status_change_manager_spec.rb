  ...
  describe "#process" do
    ...
    context "user_submitted" do
      ...
      it "doesn't send admin email if not a new record" do
        tile = FactoryBot.build :multiple_choice_tile, status: Tile::USER_SUBMITTED,  demo: demo, creator: user, user_created: true
      ...
      it "doesn't sends emails status not user_submitted" do
        tile = FactoryBot.build :multiple_choice_tile,  status: Tile::DRAFT, demo: demo, creator: user, user_created: true
        do_no_admin_email_sent tile
      end

      it "doesn't sends emails if user_created is nil" do
        tile = FactoryBot.build :multiple_choice_tile, status: Tile::USER_SUBMITTED, demo: demo, creator: user, user_created: nil
        do_no_admin_email_sent tile
      end

      it "doesn't sends emails if user_created is false" do
        tile = FactoryBot.build :multiple_choice_tile, status: Tile::USER_SUBMITTED, demo: demo, creator: user, user_created: false
        tile.user_created =  false
        do_no_admin_email_sent tile
      end

      it "doesn't sends emails if it has no creator" do
        tile = FactoryBot.build :multiple_choice_tile, status: Tile::USER_SUBMITTED, demo: demo, creator: nil, user_created: true
        do_no_admin_email_sent tile
      end
    end
    ...
    context "accepted" do
      ...
      it "doesn't sends emails original status not USER_SUBMITTED" do
        tile = FactoryBot.create :multiple_choice_tile, status: Tile::USER_DRAFT, demo: demo, creator: user, user_created: true
        tile.status = Tile::DRAFT
        mgr = SuggestedTileStatusChangeManager.new(tile)
        mgr.expects(:send_acceptance_email).never
        mgr.process
      ...
      context "is not user created" do
        let(:tile) { FactoryBot.create :multiple_choice_tile, status: Tile::USER_DRAFT, demo: demo, creator: user, user_created: nil  }
        it "doesn't sends emails if user_created is nil" do
          tile.status = Tile::DRAFT
          mgr = SuggestedTileStatusChangeManager.new(tile)
          mgr.expects(:send_acceptance_email).never
      ...
      it "doesn't sends emails if it has no creator" do
        tile = FactoryBot.create :multiple_choice_tile, status: Tile::USER_SUBMITTED, demo: demo, creator: nil, user_created: true
        tile.status = Tile::DRAFT
        mgr = SuggestedTileStatusChangeManager.new(tile)
        mgr.expects(:send_acceptance_email).never
        mgr.process
    ...
    context "posted" do
      ...
      context "original status DRAFT" do
        let(:tile) { FactoryBot.create :multiple_choice_tile, status: Tile::DRAFT, demo: demo, creator: user, user_created: true }
      ...
      it "doesn't sends emails original status not DRAFT" do
        tile = FactoryBot.create :multiple_choice_tile, status: Tile::USER_SUBMITTED, demo: demo, creator: user, user_created: true
        tile.status = Tile::ACTIVE
        mgr = SuggestedTileStatusChangeManager.new(tile)
        mgr.expects(:send_posted_email).never
        mgr.process
      ...
      it "doesn't sends emails if has no creator" do
        tile = FactoryBot.create :multiple_choice_tile, status: Tile::DRAFT, demo: demo, creator: nil, user_created: true
        tile.status = Tile::ACTIVE
        mgr = SuggestedTileStatusChangeManager.new(tile)
        mgr.expects(:send_posted_email).never
        mgr.process
