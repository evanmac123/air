...
describe InsertTileBetweenTiles do
  ...
  describe "#insert!" do
    it "should work with left and right tiles" do
      ...
      left_tile = tiles[3]
      right_tile = tiles[2]
      InsertTileBetweenTiles.new(tile, left_tile.id, right_tile.id, Tile::DRAFT).insert!
      tile.reload
    ...
    it "should work with only left tile" do
      ...
      tile = tiles[3]
      left_tile = tiles[0]
      InsertTileBetweenTiles.new(tile, left_tile.id, nil, Tile::DRAFT).insert!
      tile.reload
      expect(tile.prev_tile_in_board).to eq(left_tile)
    end
    ...
    it "should work only with right tile" do
      ...
      tile = tiles[1]
      right_tile = tiles[3]
      InsertTileBetweenTiles.new(tile, nil, right_tile.id, Tile::DRAFT).insert!
      tile.reload
      expect(tile.next_tile_in_board).to eq(right_tile)
    end
    ...
    it "should work if section is empty" do
      tile = FactoryBot.create(:multiple_choice_tile, :active, demo: demo)
      InsertTileBetweenTiles.new(tile, nil, nil, Tile::DRAFT).insert!
      tile.reload
      expect(tile.status).to eq(Tile::DRAFT)
