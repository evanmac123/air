...
module Reporting
  module Db
    ...
    describe TileActivity do
      ...
      before do
        t = Time.zone.local(2016, 02, 5, 20, 0, 0)  #FIXME this fails for Feb 28, 2016 at after 11pm  WTF?????
        Timecop.freeze(t)
        setup_data
        @tar = TileActivity.new(@demo.id, 8.weeks.ago, Time.zone.now, "week")
      end
      ...
      context "#run with demo" do
        ...
        context "Tile Activity" do
          ...
          it "calcs completion percent" do
            skip "data aggregations vary based on report date"
