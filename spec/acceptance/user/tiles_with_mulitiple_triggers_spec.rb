...
feature 'User interacts with single- and multiple-trigger tiles' do
  ...
  before(:each) do
    ...
    FactoryGirl.create :rule_trigger, tile: single_tile, rule: fish_rule

    FactoryGirl.create :rule_trigger, tile: or_tile, rule: walk_rule
    FactoryGirl.create :rule_trigger, tile: or_tile, rule: run_rule

    FactoryGirl.create :rule_trigger, tile: and_tile, rule: beans_1_rule
    FactoryGirl.create :rule_trigger, tile: and_tile, rule: beans_2_rule
    FactoryGirl.create :rule_trigger, tile: and_tile, rule: beans_3_rule
  end
  ...
  def current_slideshow_tile
    ...
    current_tile = all('#slideshow img').sort_by { |img| img[:style].slice(/(z-index: )(\d)/, 2) }.reverse.first
