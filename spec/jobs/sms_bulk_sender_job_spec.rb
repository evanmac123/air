...
RSpec.describe SmsBulkSenderJob, type: :job do
  ...
  it "executes a SmsSenderJob for each user" do
    ...
    SmsSenderJob.expects(:perform_now).with(to_number: user_1.phone_number, body: "body", from_number: demo.phone_number)
    SmsSenderJob.expects(:perform_now).with(to_number: user_2.phone_number, body: "body", from_number: demo.phone_number)

    SmsBulkSenderJob.perform_now(user_ids: User.pluck(:id), body: "body", from_number: demo.phone_number)
  end
end
