module ClientAdmin::TilesHelper
  ...
  def email_site_link(user, demo, is_preview = false)
    _demo_id = demo.kind_of?(Demo) ? demo.id : demo

    email_link_hash = is_preview ? {demo_id: _demo_id} : { protocol: email_link_protocol, host: email_link_host, demo_id: _demo_id }
  ...
  def digest_email_heading_begin
    @follow_up_email ? 'Here' : 'Your'
  end

  def digest_email_heading_link
    @follow_up_email ? 'the tiles' : 'new tiles'
  end

  def digest_email_heading_end
    @follow_up_email ? 'you missed' : 'are here'
  end
