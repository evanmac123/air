...
$(() => {
  $("#search_for_referrer #autocomplete").on("keypress", () => {
    const email = $("#user_email").val();
    const callingDiv = "#search_for_referrer";
    startWatchDog({ email, callingDiv });
    markForSend();
  });

  $("#search_for_referrer .single_suggestion").on("click", () => {
    stopWatchDog();
    $("#user_game_referrer_id").val(
      $(this)
        .find(".suggested_user_id")
        .text()
    );
  ...
  $(".remove_referrer").on("click", () => {
    $("#user_game_referrer_id").val("");
    $(this)
      .parent(".single_suggestion")
      .remove();
    $("#autocomplete").show();
    $("#autocomplete").val("");
    $("#autocomplete").focus();
