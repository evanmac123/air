...
class BulkLoad::UserCreatorFromCsv
  ...
  protected
    ...
    def add_column!(column_name, value, new_user_attributes)
      ...
      elsif is_population_segment?(column_name)
        add_population_segment! column_name, value, new_user_attributes
      else
    ...
    def is_population_segment?(name)
      name =~ /^segment_\d+$/
    end
    ...
    def clean_attributes_for_existing_user(user, new_user_attributes)
      ...
      check_if_user_has_changed_email result, user
    end
