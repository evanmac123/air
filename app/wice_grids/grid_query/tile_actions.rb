# Deprecate this class and move to front end grid solution.
  ...
  protected
    ...
    def all
      ...
      tile.demo.users.select(
        ...
        "LEFT OUTER JOIN (#{users_completions_subquery}) AS tile_completions ON tile_completions.user_id = users.id"
      ).joins(
        "LEFT OUTER JOIN (#{users_completions_subquery}) AS tile_completions ON tile_completions.user_id = users.id"
      ).joins(
        "LEFT OUTER JOIN (#{users_viewings_subquery}) AS tile_viewings ON tile_viewings.user_id = users.id"
      )
    end
end
