...
class DefaultClaimStateMachine < ClaimStateMachine
  ...
  def find_claim_state(claim_state_id)
    case claim_state_id
    when 1
      ClaimState.new(finder_method: :by_claim_code, next_state_on_ambiguity_id: 2, ambiguity_message: "Sorry, we need a little more information to figure out who you are. Please send your 5-digit home ZIP code.", unrecognized_information_message: "I can't find your number in my records. Did you claim your account yet? If not, text your first initial and last name (if you are John Smith, text \"jsmith\").", already_claimed_message: %(That ID "@{claim_information}" is already taken. If you're trying to register your account, please text in your own ID first by itself.))
    when 2
