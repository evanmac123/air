...
=end
  ...
  def daily_data(start_date, end_date, acts, users)
    ...
    plot_acts = self.acts.where(created_at: date_range)
  ...
  def hourly_data(date, acts, users)
    ...
    plot_acts = self.acts.where(created_at: hour_range)
  ...
  # todo make this a class, pass demo along with other params and have daily? method

  def highchart(start_date, end_date = nil, acts, users)
    ...
    return "Nothing to plot" if ( ! (acts or users) or (self.acts.count == 0) )
    ...
    LazyHighCharts::HighChart.new do |hc|
      ...
      hc.series(name: 'All Acts',    data: act_data)    if acts
      hc.series(name: 'Unique Users', data: user_data) if users
    end
  end
end