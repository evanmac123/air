...
module Concerns::Attachable
 ...
 # TODO consider changing this implentation to simply use the file_attachments
  ...
  def copy_attachment object, filename, target
    ...
    object.copy_to(new_key, {:acl => :public_read})
    target.file_attachments[filename]= "/#{new_key}"
  rescue
    Rails.logger.warn("Exception raised while copying tile attachmet orig-key: #{object.key} : copy-key #{new_key}")
  end

  def delete_s3_attachments
    #NOTE use compact to remove nils
  ...
  def get_s3_object s3_key
    ...
    Rails.logger.warn "#{e.message} -- #{s3_key}"
    nil
  end
