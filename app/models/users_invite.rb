      ...
      User.transaction do
        users.each do |user|
          if !user.is_a?(Hash)
            ...
            user.invite(from_user, custom_message: message, custom_from: from_user.email_with_name_via_airbo, demo_id: demo_id) #mail is sent regardless of transaction succeeded or not, but should never happen
          end
        end
      end
    end
