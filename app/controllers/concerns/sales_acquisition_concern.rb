...
module SalesAcquisitionConcern
  ...
  private
    ...
    def ping_lead_activation(user)
      user.redis[:invite_link_click_count].call(:set, 1)
      event = "Acquisition - Activated via invite link"
      ping_parameters = { link_click_count: user.redis[:invite_link_click_count].call(:get) }

      ping(event, ping_parameters, user)
    end

    def ping_lead_return_via_invite_link(user)
      user.redis[:invite_link_click_count].call(:incr)
