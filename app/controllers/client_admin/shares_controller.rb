class ClientAdmin::SharesController < ClientAdminBaseController
  def show
    ...
    @digest_tiles = @demo.digest_tiles(@tile_digest_email_sent_at)#.where(id: @demo.tiles.active.first.id)
  ...
  def show_first_active_tile
    @demo = current_user.demo
    @tile_digest_email_sent_at = @demo.tile_digest_email_sent_at
    @digest_tiles = @demo.digest_tiles(@tile_digest_email_sent_at)#.where(id: @demo.tiles.first.id)
    render :layout => false
  end
end
