...
$(function() {
  ...
  $("#search_for_friends_to_invite #autocomplete").on("keypress", function() {
    ...
    var options = {
      email: email,
      calling_div: "#search_for_friends_to_invite"
    };
    startWatchDog(options);
    markForSend();
  });
...
function autocompleteIfNoneRunning(options) {
  ...
  if (autocomplete_waiting) {
    ...
    setTimeout(function() {
      autocompleteIfNoneRunningAndResetQueue(options);
    }, 1000);
  } else {
  ...
  if (entered_text.length > 2) {
    ...
    $.get("/invitation/autocompletion#index", options, function(data) {
      ...
      if (data == "") {
        ...
        $("#search_for_referrer #autocomplete_status").text(
          "Click on the person who referred you:"
        );
