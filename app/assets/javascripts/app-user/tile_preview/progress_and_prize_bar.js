...
Airbo.ProgressAndPrizeBar = (function() {
  ...
  function fillBarToFinalProgress(finalProgress, allTilesDone, callback) {
    ...
    return $({ progressCount: 0 }).animate(
      { progressCount: stepsNumber },
      {
        duration: 250,
        easing: "linear",
  ...
  function predisplayAnimations(tileData, response) {
    ...
    return $.when(
      fillTileBar(tileData.all_tiles, tileData.completed_tiles)
    ).then(function() {
      //third, animate total points. fourth, animate raffle antries
      return $.when(
        animateCounter(
          "total_points",
          startingData.starting_points,
          tileData.ending_points,
          0.25
        )
      ).then(function() {
        if (radialProgressBar.length > 0) {
          return fillBar(
            startingData.starting_tickets,
            tileData.ending_tickets,
            tileData.raffle_progress_bar,
            tileData.all_tiles_done
          );
        }
      });
    });
  ...
  function initLocalUserProgress() {
    progress = Airbo.LocalStorage.get(config.key) || {
      available: config.tileIds,
      completed: {},
  ...
  function completedTileIds() {
    return keys(progress.completed).map(function(num) {
      return parseInt(num);
    });
  }
  ...
  function init() {
    ...
    bindShowMoreTilesLink(
      ".show_more_tiles",
      ".tile-wrapper",
      "#show_more_tiles_spinner",
      ".show_more_tiles_copy",
      "#tile_wall",
      "replace",
      onMoreTilesDisplayed
    );
  }
  return {
