...
Airbo.Highcharts.CustomTooltips.SharedTooltipWithPercentChange = (function() {
  ...
  function defaultSeriesNameComponent(data) {
    return (
      "<p style='margin:0;color:#33445c;min-width:250px;font-size:15px;font-weight:bold;'><span style='color:" +
      data.color +
      "'>\u25CF </span>" +
      data.series.name +
      "</p>"
  ...
  function defaultCountComponent(data) {
    return (
      "<span style='margin-left:12px;color:#33445c;font-size:13px;'>" +
      Highcharts.numberFormat(data.y, 0, "", ",") +
      "</span>"
    );
  }
    ...
    if (percentChange !== undefined) {
      if (percentChange === Infinity) {
        ...
        if (percentChange >= 5000) {
          decoratedChange =
            "+" +
            Highcharts.numberFormat(Math.round(point.y / prevY), 0, "", ",") +
            "X";
        } else {
          decoratedChange =
            "+" + Highcharts.numberFormat(percentChange, 0, "", ",") + "%";
        }
      } else {
