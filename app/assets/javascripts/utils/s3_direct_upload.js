...
Airbo.DirectToS3FileUploader = (function() {
  var uploadTypes = {
    tileImage: {
      ...
      messages: {
        acceptFileTypes:
          "File type not allowed. Must be a gif, bmp, jpeg, or png image file",
    ...
    tileAttachment: {
      ...
      messages: {
        acceptFileTypes:
          "File type not allowed. Must be a pdf or MS word document file",
  ...
  function handleAnyFileErrors(file) {
    if (file.error) {
      alert(
        "Sorry unable to upload the file due to the following error: " +
          file.error
      );
    }
  }
  ...
  function initFileUploader() {
    $(".s3-uploader").each(function() {
      var form = $(this);
      var type = form.data("uploadtype");
      var config = uploadTypes[type];
      var eventPrefix = "/s3/" + type + "/upload/";
