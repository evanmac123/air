...
Airbo.ExploreDigest = (function() {
  ...
  function addNewFeatureTemplate(count) {
    $(".features-container").append(
      '<div class="panel feature-section"><legend>Feature ' +
        count +
        '</legend><br><div class="add-feature-form"><label for="feature_' +
        count +
        '_headline">Headline</label><input id="feature_' +
        count +
        '_headline" name="features[' +
        count +
        '][headline]" type="text"><label for="feature_' +
        count +
        '_feature_message">Feature Message</label><textarea columns="60" id="feature_' +
        count +
        '_feature_message" name="features[' +
        count +
        '][feature_message]" rows="5"></textarea><label for="feature_' +
        count +
        '_tile_ids">Tile Ids (in order and comma separated)</label><input id="features_' +
        count +
        '_tile_ids" name="features[' +
        count +
        '][tile_ids]" placeholder="1, 2, 3, 4, 5, 6" type="text"></div></div>'
    );
  }
  ...
  function initMailerForms() {
    ...
    $("#send_targeted_digest").on("click", function(e) {
      ...
      if (confirmed === true) {
        ...
        var params = {
          test_digest: false,
          targeted_digest: { send: true, users: user_ids }
        };
        deliver(path, self, params);
      }
    });
