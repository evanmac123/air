...
Airbo.SignUpFromPreview = (function() {
  var modalObj = Airbo.Utils.StandardModal(),
    ...
    blockedElements = [".see_airbo", ".tag"].join(", ");
    ...
    if (data.status == "success") {
      if (window.pathForActionAfterRegistration) {
        localStorage.setItem(
          "pathForActionAfterRegistration",
          window.pathForActionAfterRegistration
        );
        location.reload();
  ...
  function dompath(element) {
    ...
    var idSelector = $(element).attr("id")
      ? $.trim($(element).attr("id"))
          .split(" ")
          .join("#")
      : "";
    var classSelector = $(element).attr("class")
      ? $.trim($(element).attr("class"))
          .split(" ")
          .join(".")
      : "";
    var eleSelector =
  ...
  function initEvents() {
    ...
    $("#create_account_form")
      .on("submit", creationStartCallback)
      .on("ajax:success", creationResponseCallback);
  }
