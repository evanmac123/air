context('Draft Tile status nav functionality', () => {
  ...
  describe('basic tile functionality', () => {
    it('displays the tiles in plan properly', function() {
      ...
      cy.get(`[data-tile-container-id=${this.tiles[0].id}] a.button.update_status`).invoke('show').click();
      ...
      cy.contains('Ready to Send').children('span').should('have.text', '(0)');
      cy.contains('Plan').children('span').should('have.text', '(2)').click();
