...
Then /^I should see the password reset full URL for "([^"]*)" in the email body$/ do |username|
  ...
  step %{I should see 'href="#{edit_user_password_url(user, :token => user.confirmation_token)}"' in the email body}
end

Then /^I should see the profile page full URL for "([^"]*)" in the email body$/ do |username|
  user = User.find_by_name(username)
  step %{I should see 'href="#{user_url(user)}"' in the email body}
end
...
Then /^"([^"]*)" should receive exactly (\d+) email containing "([^"]*)"$/ do |address, count, expected_text|
  count = count.to_i
  unread_emails_for(address).select{|email| email.to_s.include?(expected_text)}.should have(count).emails
end
