IntercomRails.config do |config|
  ...
  #
  config.api_secret = ENV["INTERCOM_SECURE_SECRET"] unless Rails.env.test?
  ...
  #
  config.user.current = Proc.new { current_user if current_user.is_a?(User) }
  ...
  #
  config.user.exclude_if = Proc.new { |user| user.end_user? }
  ...
  #
  config.user.custom_data = Proc.new { |user| user.intercom_data }
  ...
  #
  # config.company.current = Proc.new { current_user.organization }
