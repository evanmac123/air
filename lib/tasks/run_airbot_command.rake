namespace :airbot do
  namespace :scheduled_task do
    ...
    task :send_expense_reminder => :environment do
      ...
      processing_coming_up = (days_left_until_processing == 2 || days_left_until_processing == 1 || ((1..2).to_a.include?(payday_processing.wday) && (days_left_until_processing == 4 || days_left_until_processing == 3)))

      if processing_coming_up
        text = if days_left_until_processing == 1 || (payday_processing.monday? && days_left_until_processing == 3)
